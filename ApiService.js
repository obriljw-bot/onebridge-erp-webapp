...
/**
 * 청구서 재출력 API
 * - 청구DB에서 발주번호 목록을 조회하여 거래명세서/발주서 재출력
 * @param {Object} params - { billingId, type }
 */
function reprintInvoiceApi(params) {
  try {
    var billingId = params.billingId || '';
    var billingType = params.type || 'SALES';
    ...
    for (var i = 1; i < data.length; i++) {
      if (data[i][colBillingId] === billingId) {
        ...
        foundType = data[i][colType] || billingType;
        break;
      }
    }
    ...
    try {
      // JSON 배열 형식인 경우
      if (orderNumbersStr.startsWith('[')) {
        orderCodes = JSON.parse(orderNumbersStr);
      } else {
        // 쉼표로 구분된 문자열인 경우
        orderCodes = orderNumbersStr.split(',')
    ...
    // 4. 문서 유형 결정
    var docType = foundType === 'PURCHASE' ? 'ORDER_PURCHASE' : 'INVOICE_VAT';
